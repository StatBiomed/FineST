...
## 2026.08.05 LLY move to package as nuclei_segmentation API

"""
StarDist nuclei segmentation for spatial transcriptomics data.

Public API::

    import FineST as fst
    fst.nuclei_segmentation(
...
Or from the terminal::
    ...
    python -m FineST.nuclei_segmentation --adata_path ... --image_path ... ...
"""
...
from .paths import apply_data_root_nuclei, normalize_data_root
...
class FineST_StarDist_nuclei:
    ...
    def crop_img_adata(self):
        ...
        if len(roi_coords) > 2 and roi_coords[2][0] == 0:
            ...
            adata_roi.obsm["spatial"] = adata_roi.obsm["spatial"] - np.array(
                [roi_coords[0][1], roi_coords[0][0]]
            )
    ...
    def DissectSegRes(df):
        ...
        for row in df.iterrows():
            ...
            for idx, i in enumerate(row[1]['segmentation_centroid']):
                tmps.append(
                    list(i) + [row[0], row[0] + '_{}'.format(idx), row[1]['segmentation_label']]
                )
        return pd.DataFrame(
    ...
    def NucleiSegmentation(self):
        ...
        for row in df_cells.iterrows():
            if row[1]['segmentation_label'] > self.max_cell_number:
                ...
                df_cells.at[row[0], 'segmentation_centroid'] = row[1]['segmentation_centroid'][
        ...
        coord_csv_path = os.path.join(self.out_dir, "position_all_tissue_sc.csv")
        coord_cell_filtered.to_csv(coord_csv_path)

        return coord_cell_filtered, coord_csv_path
...
def nuclei_segmentation(
    ...
    *,
    ...
    max_cell_number: int = 20,
    min_counts: int = 500,
) -> Tuple[anndata.AnnData, str, pd.DataFrame, str]:
    """Run StarDist nuclei segmentation on H&E image and ST coordinates.

    Wrapper around :class:`FineST_StarDist_nuclei` for notebook / package use.

    Parameters
    ----------
    adata_path : str
        Path to spot-level AnnData (.h5ad).
    image_path : str
        Path to H&E image (.tif, .btf, etc.).
    prob_thresh : float
        StarDist probability threshold (default 0.5).
    save_folder : str
        Output subdirectory name under ``out_dir`` (e.g. ``NPC_allspot_p075``).
    out_dir : str
        Base output directory.
    roi_path : str, optional
        ROI coordinates CSV from Napari. If None, process the full image.
    max_cell_number : int
        Maximum nuclei assigned per spot (default 20).
    min_counts : int
        Minimum UMI count threshold reserved for future filtering (default 500).

    Returns
    -------
    sp_adata : AnnData
        AnnData with segmentation features and cell locations.
    tissue_out_dir : str
        Output directory for this tissue sample.
    coord_cell_filtered : pd.DataFrame
        Single-cell coordinates mapped to spots.
    coord_csv_path : str
        Path to ``position_all_tissue_sc.csv``.
    """
    segmenter = FineST_StarDist_nuclei(
        save_folder,
        out_dir,
        roi_path,
        image_path,
    ...
    coord_cell_filtered, coord_csv_path = segmenter.NucleiSegmentation()
    return segmenter.sp_adata, segmenter.out_dir, coord_cell_filtered, coord_csv_path


main = nuclei_segmentation


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='FineST StarDist nuclei segmentation')
    parser.add_argument(
        '--data_root',
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default='HIPT',
        help='HIPT or Virchow2 (used with --data_root for preset paths)',
    )
    parser.add_argument(
        '--save_folder',
        '--tissue',
        type=str,
        required=True,
        help='output subdirectory name (alias: --tissue)',
    )
    parser.add_argument('--out_dir', type=str, default=None, help='base output directory')

    ## my add
    parser.add_argument('--roi_path', type=str, default=None, help='ROI path')
    parser.add_argument('--adata_path', type=str, default=None, help='ST data path')
    parser.add_argument(
        '--image_path',
        '--img_path',
        type=str,
        default=None,
        help='H&E stained image data path (alias: --img_path)',
    )
    ...
    parser.add_argument(
        '--max_cell_number', type=int, default=20, help='maximum cell number per spot'
    )
    parser.add_argument(
        '--min_counts', type=int, default=500, help='minimum UMI count per spot'
    )
    args = parser.parse_args()

    if args.data_root:
        args = apply_data_root_nuclei(
            args, normalize_data_root(args.data_root), hist_model=args.hist_model
        )
    missing = []
    if not args.out_dir:
        missing.append('--out_dir')
    if not args.adata_path:
        missing.append('--adata_path')
    ...
    if missing:
        parser.error(
            ...
            + ', '.join(missing)
            + ' (or pass --data_root to fill defaults)'
        )
...
#     image_path='./Dataset/CRC16um/Visium_HD_Human_Colon_Cancer_tissue_image.btf',
#     save_folder='CRC16um_ROI_test',
