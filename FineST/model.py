...
def compute_pearson_corr(x, y, dim):
    """
    Function to compute Pearson correlation coefficient and convert it to loss.

    :param x: torch.Tensor, the first input tensor
    :param y: torch.Tensor, the second input tensor
    :param dim: int, the dimension to compute the correlation over
...
def PearsonCorrelationLoss(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    """
    Function to compute the Pearson Correlation loss between two tensors.
